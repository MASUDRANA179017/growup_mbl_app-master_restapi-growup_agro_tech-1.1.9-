...
import 'package:growup_agro/models/property_details_model.dart';
...
class PropertiesDetails extends StatefulWidget {
  final num? projectId;
  const PropertiesDetails({super.key, this.projectId});
...
class _PropertiesDetailsState extends State<PropertiesDetails> {
  ...
  Future<void> _fetchProjectDetails() async {
    ...
    final url = Uri.parse(
      ApiConstants.allPropertiesDetails(
        investorCode,
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _project = PropertyDetailsModel.fromJson(jsonDecode(response.body));
        setState(() {
          // _project = data;
          _loading = false;
        });
  ...
  Widget build(BuildContext context) {
    final project = _project?.properties;
    ...
    return Scaffold(
      ...
      body: _loading
          ...
          : project == null
          ? const Center(child: Text('Property data not available'))
          : RefreshIndicator(
          ...
          child: Column(
            ...
            children: [
              ...
              model!.imageUrl!.isNotEmpty)
                networkImage(model.imageUrl, height: 180, width: double.infinity),
                // ClipRRect(
              ...
              _buildSummaryCard(_project!),
              const SizedBox(height: 16),

              // if (project.securityDetails.isNotEmpty)
              //   Column(
              //     crossAxisAlignment: CrossAxisAlignment.start,
              //     children: [
  ...
  Widget _buildSummaryCard(PropertyDetailsModel project) {
    // final nf = NumberFormat.decimalPattern();
    final properties = project.properties;
    String showAmount(String? amount){
      return '${amount ?? 0} TK';
    }
    return Container(
      decoration: _cardBoxDecoration(),
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          // InfoRow(title: 'Project Name', value: properties?.projectName),
          // InfoRow(title: 'Business Type', value: properties?.businessTypeName),
          // InfoRow(
          //   title: 'Investment Time',
          //   value: '${project.investmentTime} days',
          // ),
          // InfoRow(
          //   title: 'Start Date',
          //   value: formatDate(properties?.projectStartDate),
          // ),
          // InfoRow(
          //   title: 'Mature Date',
          //   value: formatDate(properties?.projectEndDate),
          // ),
          // InfoRow(
          //   title: 'ROI Start Date',
          //   value: formatDate(properties?.roiStartDate),
          // ),
          InfoRow(
            title: 'Investment Goal',
            // value: '${nf.format(properties?.investmentGoal ?? 0)} Tk',
            value: showAmount(properties?.investmentGoal),
          ),
          InfoRow(
            title: 'Min Investment',
            // value: '${nf.format(properties?.minInvestmentAmount ?? 0)} Tk',
            value: showAmount(properties?.minInvestmentAmount),
          ),
          // InfoRow(
          //   title: 'In Waiting',
          //   value: showAmount(project.inWaiting),
          //   // value: '${nf.format(project.inWaiting)} Tk',
          // ),
          // InfoRow(title: 'Raised',
          //     value: showAmount(project.rasied),
          //     // value: '${nf.format(project.rasied)} Tk',
          // ),
          // InfoRow(title: 'ROI (%)', value: 'Annually ${properties?.annualRoi}%'),
          // InfoRow(
          //   title: 'Project Duration',
          //   value: '${properties?.projectDurationViewer} Months',
          // ),
  ...
  // Widget _buildSecurityCard(PropertyDetailsModel project) => Container(
  //   decoration: _cardBoxDecoration(),
  //   padding: const EdgeInsets.all(12),
  //   child: Column(
  //     crossAxisAlignment: CrossAxisAlignment.start,
  ...
  Future<void> _triggerInvestDialog() async {
    ...
    final project = _project!;
    final model = _project?.properties;

    final controller = TextEditingController(
      text: double.tryParse(
        model?.minInvestmentAmount?.toString() ?? '0',
      )
          ?.toInt()
          .toString() ??
          '0',
          ...
          builder: (ctx, setState) => AlertDialog(
            ...
            content: Column(
              ...
              children: [
                ...
                Text("Min. Investment: ৳${model?.minInvestmentAmount ?? 0}"),
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: loading
                  ...
                  if (amount < (double.tryParse('${model?.minInvestmentAmount ?? 0}') ?? 0).toInt()) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text(
  ...
  Future<Map<String, dynamic>?> _showInvestDialog({
    ...
    final walletBalance = await _fetchLatestWalletBalance(token, investorCode);
    final project = _project!;
    final properties = _project!.properties;
        ...
        return StatefulBuilder(
          builder: (context, setState) {
            final amountController = TextEditingController(
              text:double.tryParse(
                properties?.minInvestmentAmount?.toString() ?? '0',
              )
                  ?.toInt()
                  .toString() ??
                  '0',
                ...
                content: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      InfoRow(
                        title: "Min. Investment",
                        value: '৳${properties?.minInvestmentAmount}',
                        fontSize: 12,
                        showDivider: false,
                      ),
                      InfoRow(
                        title: "In Waiting",
                        value: '৳${project.inWaiting}',
                        fontSize: 12,
                        showDivider: false,
                      ),
                      InfoRow(
                ...
                actions: [
                  ...
                  CustomButton(
                    ...
                    onPressed: _isLoading
                      ...
                      if (amount < (num.tryParse(properties?.minInvestmentAmount ?? '0') ?? 0)) {
                        if (!mounted) return;
                        ScaffoldMessenger.of(outerContext).showSnackBar(
                          SnackBar(
                            content: Text(
                              "Invest amount must be at least ৳${properties?.minInvestmentAmount}",
                            ),
                            backgroundColor: Colors.red,
                          ),
                        );
                      ...
                      // if ((num.tryParse(properties?.minInvestmentAmount ?? '0') ?? 0).toInt() >
                      //     (num.tryParse(project.inWaiting ?? '0') ?? 0).toInt()) {
                      //   log('min invest: ${properties?.minInvestmentAmount}--->> waiting: ${project?.inWaiting}');
                      //   if (!mounted) return;
                      //   ScaffoldMessenger.of(outerContext).showSnackBar(
                      //     const SnackBar(
                      //       content: Text(
